<div align="center" style="
  background: radial-gradient(circle at center, #000000 30%, #0a0a0a 100%);
  color: #ffd700;
  padding: 50px;
  border-radius: 18px;
  border: 2px solid #ffcc00;
  box-shadow: 0 0 40px #ffcc00, inset 0 0 30px rgba(255,204,0,0.3);
  font-family: 'Orbitron', 'Consolas', monospace;
  text-shadow: 0 0 10px #ffcc00aa, 0 0 20px #ffcc00aa;
  transition: all 0.3s ease-in-out;
">

  <pre style="color:#ffae00; font-size:14px; margin-bottom:0;">[ KI SYSTEM ONLINE... POWER CHARGING ⚡ ]</pre>

  <h1 style="margin:10px 0 0; font-size:52px; letter-spacing:2px;
    background:linear-gradient(90deg,#fff5a0,#ffcc00,#ffea70);
    -webkit-background-clip:text; color:transparent; text-shadow:none;">
    YOUNG LORD // SUPER SAIYAN MODE
  </h1>

  <img src="https://i.giphy.com/8oDwbeUhL0C6IylHD1.webp" width="240"
    style="margin:25px 0 15px; border-radius:10px; box-shadow:0 0 25px #ffcc00aa, 0 0 40px #ffcc00aa;" />

<pre style="
  color: #fff59d; 
  font-size: 13px; 
  font-family: 'Courier New', monospace; 
  background: rgba(255, 215, 0, 0.08); 
  border: 1px solid rgba(255, 215, 0, 0.3); 
  box-shadow: 0 0 12px rgba(255, 215, 0, 0.4);
  padding: 10px 14px;
  border-radius: 8px;
  text-shadow: 0 0 5px #ffeb3b, 0 0 10px #ffd54f;
">
>>> Channeling KI Energy ... SUCCESS  
>>> Energy Level ... ████████████████████ 9000+
</pre>

  <hr style="border:0; height:2px; width:90%;
    background:linear-gradient(90deg,#00ffff00,#00ffff88,#ff00ff66,#00ffff00);
    margin:40px 0;" />

  <h2 style="color:#ffea70; text-transform:uppercase; font-size:22px;">[ SUPER SAIYAN TRANSFORMATION ]</h2>

  <div style="max-width:850px; text-align:left; background:#0b0b0b;
    border:1px solid #ffcc00; border-radius:12px; padding:25px;
    box-shadow:0 0 15px #ffcc00aa; margin-top:15px;">
    <p><strong>Ki Core:</strong> Build | Explore | Conquer</p>
<p><strong>Saiyan Awakening:</strong> HTML → CSS → JavaScript → React → Node.js → MongoDB</p>
<p><strong>God Moves:</strong> Socket.io | Redis | Firebase | Zustand | Redux | Tailwind | gRPC | RabbitMQ | BullMQ</p>
<p><strong>Hidden Genes:</strong> Mechanical Engineering + Computer Systems</p>
  </div>

  <hr style="border:0; height:2px; width:80%;
    background:linear-gradient(90deg,#39ff1400,#39ff1488,#00fff788,#ff00ff00);
    margin:40px 0;" />

  <h2 style="color:#ffe066;">[ TECHNIQUES MASTERED ]</h2>

  <div style="display:flex; flex-wrap:wrap; justify-content:center; gap:15px; margin-top:25px;">
  <img src="https://img.shields.io/badge/JavaScript-000?style=for-the-badge&logo=javascript&logoColor=F7DF1E" />
  <img src="https://img.shields.io/badge/C++-000?style=for-the-badge&logo=cplusplus&logoColor=6295CB" />
  <img src="https://img.shields.io/badge/React-000?style=for-the-badge&logo=react&logoColor=61DAFB" />
  <img src="https://img.shields.io/badge/TailwindCSS-000?style=for-the-badge&logo=tailwindcss&logoColor=38BDF8" />
  <img src="https://img.shields.io/badge/Node.js-000?style=for-the-badge&logo=nodedotjs&logoColor=00ff00" />
  <img src="https://img.shields.io/badge/Express-000?style=for-the-badge&logo=express&logoColor=white" />
  <img src="https://img.shields.io/badge/MongoDB-000?style=for-the-badge&logo=mongodb&logoColor=47A248" />
  <img src="https://img.shields.io/badge/Redis-000?style=for-the-badge&logo=redis&logoColor=DC382D" />
  <img src="https://img.shields.io/badge/AWS-000?style=for-the-badge&logo=amazonaws&logoColor=FF9900" />
  <img src="https://img.shields.io/badge/RabbitMQ-000?style=for-the-badge&logo=rabbitmq&logoColor=FF6600" />
  <img src="https://img.shields.io/badge/BullMQ-000?style=for-the-badge&logo=redis&logoColor=DC382D" />
  <img src="https://img.shields.io/badge/Redux-000?style=for-the-badge&logo=redux&logoColor=764ABC" />
  <img src="https://img.shields.io/badge/Zustand-000?style=for-the-badge&color=000000&labelColor=000000&logo=vercel&logoColor=EBCB8B" />
  <img src="https://img.shields.io/badge/Socket.io-000?style=for-the-badge&logo=socketdotio&logoColor=FFFFFF" />
  <img src="https://img.shields.io/badge/gRPC-000?style=for-the-badge&logo=googlecloud&logoColor=00C853" />
</div>


  <hr style="border:0; height:2px; width:90%;
    background:linear-gradient(90deg,#00ffff00,#00ffff88,#ff00ff66,#00ffff00);
    margin:40px 0;" />

  <h2 style="color:#ffea70;">[ HYPERBOLIC TIME CHAMBER ]</h2>

<div align="center" style="margin-top:25px;">
  <a href="https://leetcode.com/yourusername"><img src="https://img.shields.io/badge/LeetCode-000?style=for-the-badge&logo=leetcode&logoColor=FFA116" /></a>
  <a href="https://codeforces.com/profile/yourusername"><img src="https://img.shields.io/badge/Codeforces-000?style=for-the-badge&logo=codeforces&logoColor=1F8ACB" /></a>
  <a href="https://www.codechef.com/users/yourusername"><img src="https://img.shields.io/badge/CodeChef-000?style=for-the-badge&logo=codechef&logoColor=white" /></a>
  <a href="https://www.geeksforgeeks.org/user/yourusername"><img src="https://img.shields.io/badge/GFG-000?style=for-the-badge&logo=geeksforgeeks&logoColor=0F9D58" /></a>
  <a href="https://www.hackerrank.com/yourusername"><img src="https://img.shields.io/badge/HackerRank-000?style=for-the-badge&logo=hackerrank&logoColor=00EA64" /></a>
  <a href="https://www.codingninjas.com/studio/profile/yourusername"><img src="https://img.shields.io/badge/Code360-000?style=for-the-badge&logo=codingninjas&logoColor=FF5722" /></a>
  <a href="https://yourportfolio.com"><img src="https://img.shields.io/badge/Portfolio-000?style=for-the-badge&logo=About.me&logoColor=ffffff" /></a>
</div>


  <hr style="border:0; height:2px; width:85%;
    background:linear-gradient(90deg,#ff00ff00,#ff00ff88,#39ff1466,#00ffff00);
    margin:40px 0;" />

  <h2 style="color:#ffe066;">[ POWER STATS ]</h2>

  <div align="center">
    <img src="https://github-readme-stats.vercel.app/api?username=yourusername&show_icons=true&theme=tokyonight&hide_border=true&bg_color=000000&title_color=ffcc00&text_color=ffffff&icon_color=ffd700" height="170" />
    <img src="https://github-readme-streak-stats.herokuapp.com/?user=yourusername&theme=tokyonight&hide_border=true&background=000000&stroke=ffd700&ring=ffd700&fire=ffd700&currStreakLabel=ffd700&sideNums=ffffff&sideLabels=ffffff" height="170" />
    <img src="https://github-readme-activity-graph.vercel.app/graph?username=yourusername&theme=react-dark&bg_color=000000&color=ffcc00&line=ffaa00&point=ffd700&area=true&hide_border=true" style="max-width:100%; margin-top:25px;" />
  </div>

  <hr style="border:0; height:2px; width:90%;
    background:linear-gradient(90deg,#00ffff00,#00ffff88,#ff00ff66,#00ffff00);
    margin:40px 0;" />

  <div align="center" style="
    background:#0b0b0b;
    border-radius:12px;
    padding:25px;
    box-shadow:0 0 30px rgba(255,204,0,0.3);
    border:1px solid #ffcc00aa;">
    <img src="https://media1.tenor.com/m/tNYus4tK5dEAAAAd/anime-computer.gif" width="300"
      style="border-radius:8px; box-shadow:0 0 35px #ffcc00aa, 0 0 10px #ffcc00aa;" />
    <p style="font-size:15px; color:#fff176; margin-top:12px;">
      "Hire me before I go <strong style='color:#ffcc00;'> Ultra Instinct </strong> on your codebase"
    </p>
  </div>

  <pre style="color:#ffcc00; font-size:14px; margin-top:30px;">[ END OF TRANSMISSION ⚡ BREAK THROUGH TO SUPER SAIYAN]</pre>

</div>
